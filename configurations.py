class Config(object):
	def __init__(self):
		...
		self.agent_actions.extend([
			{'intent':'inform','inform_slots':{slot:'PLACEHOLDER'},'request_slots':{}} for slot in self.agent_inf_slots if slot != self.usim_def_key])
		self.agent_actions.extend([
			{'intent':'request','inform_slots':{},'request_slots':{slot:'UNK'}} for slot in self.agent_req_slots])
